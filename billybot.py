...
async def time_status():
	while True:
		...
		try:
			...
			print(e)
			print("could not change time status lol")
...
@bot.event
async def on_ready():
	#check if db is ready, create if not
	database.initial_startup()
	...
	if index:
		...
		try:
			...
			if index.startswith("<@"):
				...
				row = database.fetch_specific_entry_from_author(str(user_id))
			else:
